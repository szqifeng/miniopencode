...
const readTool = {
    ...
    async execute({ path: targetPath }) {
        try {
            ...
            return { output: `读取失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const writeTool = {
    ...
    async execute({ path: targetPath, content }) {
        try {
            ...
            return { output: `写入失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const editTool = {
    ...
    async execute({ path: targetPath, oldString, newString }) {
        try {
            ...
            return { output: `编辑失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const excelInspectTool = {
    ...
    async execute({ path: targetPath, sheet, maxRows = 20 }) {
        try {
            ...
            return { output: `读取 Excel 失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const excelWriteTool = {
    ...
    async execute({ path: targetPath, sheet, rowsJson, mode = 'replace' }) {
        try {
            const resolvedPath = resolveToolPath(targetPath);
            await fs.mkdir(path.dirname(resolvedPath), { recursive: true });
            ...
            if (mode === 'append' && workbook.Sheets[sheet]) {
                const existingRows = XLSX.utils.sheet_to_json(workbook.Sheets[sheet], {
                    defval: '',
                    raw: false
                });
            ...
            return { output: `写入 Excel 失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const csvInspectTool = {
    ...
    async execute({ path: targetPath, delimiter = ',', maxRows = 20 }) {
        try {
            ...
            return { output: `读取 CSV 失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const csvWriteTool = {
    ...
    async execute({ path: targetPath, rowsJson, delimiter = ',', mode = 'replace' }) {
        try {
            const resolvedPath = resolveToolPath(targetPath);
            await fs.mkdir(path.dirname(resolvedPath), { recursive: true });
            const incomingRows = parseStructuredRows(rowsJson);
            ...
            return { output: `写入 CSV 失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const execAsync = promisify(exec);
async function searchFiles(dir, pattern, include, results = [], depth = 0) {
    if (depth > 10)
        return results;
    try {
...
const grepTool = {
    ...
    async execute({ pattern, path: searchPath, include }) {
        try {
            ...
            return { output: `搜索失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const bashTool = {
    ...
    async execute({ command, cwd }) {
        try {
            ...
            return { output: `命令执行失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
...
const webfetchTool = {
    ...
    async execute({ url, format = 'markdown' }) {
        try {
            ...
            return { output: `获取失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
...
const writeTodoTool = {
    ...
    async execute({ content, status, id }) {
        try {
            ...
            return { output: `操作失败: ${error.message}`, title: '错误', metadata: { error: error.message } };
        }
    }
};
const readTodoTool = {
