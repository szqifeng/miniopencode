...
function resolveToolPath(targetPath: string): string {
  ...
  if (workspaceDir) {
    ...
    throw new Error(
      `路径越界：${targetPath} 不在当前 workspace 内。workspaceDir=${base}`
    );
  }
...
function resolveWorkspacePath(
  ...
  options?: {
    /**
     * 当 targetPath 只是文件名（basename）时，是否默认从 `workspaceDir/uploads/` 下解析。
     * 用于“上传文件”的读取类工具，避免前端/agent 反复拼接 uploads 前缀。
     */
    preferUploadsForBasename?: boolean;
  }
): string {
  const workspaceDir = getCurrentWorkspaceDir();
  if (!workspaceDir) {
