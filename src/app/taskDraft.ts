...
import type { TaskScheduleConfig } from './types.js';

type DraftInput = Partial<Pick<Task, 'name' | 'inputFilePath' | 'schedule' | 'scheduleConfig' | 'scheduleTime' | 'analysisGoal'>>;
type TaskDraftSchedule = Task['schedule'] | '';

export interface TaskDraftResolveParams {
...
function inferTaskName(goal?: string, inputFilePath?: string): string | undefined {
...
function normalizeTimeExpression(text: string): string | undefined {
...
function extractWeeklyDay(text: string): number | undefined {
...
function extractHourlyMinute(text: string): number | undefined {
...
function buildScheduleTime(schedule: Task['schedule'], config?: TaskScheduleConfig): string {
  if (schedule === 'manual') {
    return '仅手动执行';
  }
  if (schedule === 'hourly') {
...
function fallbackResolveTaskDraft({ messages, draft }: TaskDraftResolveParams): TaskDraftResolveResult {
  ...
  const result: TaskDraftResolveResult = {
    name: draft?.name || '',
...
function sanitizeDraftResult(input: Record<string, unknown>, fallback: TaskDraftResolveResult): TaskDraftResolveResult {
  ...
  return {
    ...
    summary: Array.isArray(input.summary) ? input.summary.filter((item): item is string => typeof item === 'string') : fallback.summary,
    missing: Array.isArray(input.missing) ? input.missing.filter((item): item is string => typeof item === 'string') : fallback.missing,
    warnings: Array.isArray(input.warnings) ? input.warnings.filter((item): item is string => typeof item === 'string') : fallback.warnings
  };
}

export async function resolveTaskDraft(params: TaskDraftResolveParams): Promise<TaskDraftResolveResult> {
  ...
  try {
    ...
    }) as { fullStream: AsyncIterable<Record<string, unknown>> };

    let text = '';
    for await (const delta of result.fullStream) {
      const event = delta as Record<string, unknown>;
      if (event.type === 'text-delta') {
        text += ((event.textDelta as string) || (event.text as string) || '');
      }
    }
...
export function getScheduleLabel(schedule: Task['schedule']) {
  if (schedule === 'hourly') {
    return '每小时';
  }
  if (schedule === 'daily') {
...
export function formatScheduleTime(schedule: Task['schedule'], config?: TaskScheduleConfig): string {
